...
define(['jquery'], function () {
    ...
    function form() {
        ...
        this.href = function (url, ele) {
            /*! 兼容模态列表分页问题 */
            if ($(ele).parents(".layui-layer-page").length > 0) {
                this.load($.menu.parseUri(url, ele), {}, 'GET', function (res) {
                    if (typeof (res) === 'object') return $.msg.auto(res),false;
                    $(ele).parents(".layui-layer-page").find(".layui-layer-content").html(res);
                    return false;
                });
                return ;
            }
