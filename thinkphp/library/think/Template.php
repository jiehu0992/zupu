...
class Template
{
    ...
    private function parseTemplateFile($template)
    {
		...
		if(!is_file($template)){
			//修复模块自身view目录下寻不到模板文件
			$temp_path = APP_PATH.Request::instance()->module().DS.basename($this->config['view_path']).DS.$template;
			$template = file_exists($template)?$template:$temp_path;
		}
        if (is_file($template)) {
            // 记录模板文件的更新时间
